...
- **Partial:** a useful subset exists, but the named capability still has an explicit gap.
- **Planned:** no complete slice exists.
...
| CodeLens | No complete equivalent found | No complete equivalent found | Delivered for available actions | Document-wide bounded reference, implementation, associated-test, and Roslyn-proven project-entry-point Run actions share the exact buffer. Queries resolve only on selection. Debug stays hidden until a real debugger adapter exists |
...
| Quick fixes, refactorings, fix-all | Code-action provider | V1/V2 refactoring services | Delivered for the closed catalog | `Ctrl+.` and Quick fix discover preflighted compiler/style fixes and local or exact-selection refactorings from pinned Roslyn providers. The menu labels document-wide fixes and atomic affected-file counts. Add Parameter and Replace Property/Method admit bounded cross-document results. Every choice carries an opaque ID and scope through complete preview/fingerprint/atomic apply; custom host operations, project/reference changes, and added files remain rejected. Models and inbound MCP share the read result; no raw action executor exists |
| Generated and metadata/decompiled source | Limited source navigation | Decompilation service | Delivered | Generated output and metadata source open through exact-buffer opaque handles as labeled read-only virtual documents. `ICSharpCode.Decompiler` reconstructs selected method bodies from exact local implementation assemblies; reference-only or unavailable bodies use an explicit signature fallback. Results are bounded and excluded from repository and layout persistence |
...
| Project User Secrets | No equivalent complete project workflow found | No equivalent complete workflow found | Delivered | Standard per-user store; bounded project-ID validation; key-only list; separate masked reveal, copy, add, change, and delete actions; atomic writes; capture/disclosure interlock; no model or MCP read tool |
| Linux editor resilience | Product-specific lifecycle | Underlying Roslyn cancellation and analyzer behavior | Delivered | The complete gate measures the current solution's cold/warm latency and retained memory; proves in-flight cancellation, analyzer degradation, repeated context replacement, keyboard and IME behavior, AT-SPI and strict Orca speech, 200% scaling, Dock restoration, and self-contained Linux x64 publication |
