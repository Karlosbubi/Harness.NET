...
| coverage import/navigation | Import a selected Cobertura report and navigate exact source-line hits | Developer UI delivered | Exact original/approved-worktree context, confined bounded XML and source mapping, durable safe provenance; no automatic report discovery or agent authority |
...
| `get_run_configurations`, `execute_run_configuration` | Discover launch profiles, executable entry points and tests; run a typed target with bounded overrides | Developer UI partial | Run CodeLens now confirms an exact inspected Project profile plus bounded process-local args/env/cwd; model execution authority and executable profiles remain planned |
