...
collaborators, exact semantic rename, and deterministic transformations are extracted
while the remaining `DocumentsHost` work continues.
...
| 060.5 — in progress | Extract **RunOutputTool**, **ProblemsTool**, **DocumentsHost** (document sessions, diagnostics views, switcher). Task 052 has no open branch ahead of `main`; its delivered CodeLens-run slice is included. Run output, Problems, the Roslyn lifecycle, editor interactions, exact rename, and deterministic transformations are extracted; navigation and session hosting remain. | `WorkbenchDockHost` ≤ 800 lines and leaves the allowlist. |
