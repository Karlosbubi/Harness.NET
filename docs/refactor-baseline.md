...
`RunOutputTool`, `ProblemsTool`, and the bounded `DocumentIntelligence` lifecycle
collaborator are extracted while the remaining `DocumentsHost` work continues.
...
| 060.5 — in progress | Extract **RunOutputTool**, **ProblemsTool**, **DocumentsHost** (document sessions, diagnostics views, switcher). Task 052 has no open branch ahead of `main`; its delivered CodeLens-run slice is included. Run output, Problems, and the Roslyn document lifecycle are extracted; document interaction, transformation, navigation, and session hosting remain. | `WorkbenchDockHost` ≤ 800 lines and leaves the allowlist. |
