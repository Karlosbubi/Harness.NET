...
| 035 | In progress | Make the workbench's daily surfaces feel like a professional IDE, not a prototype | 016, 026, 029, 032 | The decorated diff viewer with inline and side-by-side modes is implemented. The Framework effective view is still one long formatted text dump (rules, full guidance-document bodies, and issues all inline with no filtering), and the Operations backup destination still requires typing an absolute path by hand instead of a native save picker. The goal-approval dialog chain is tracked separately as Task 040. | Framework's effective view lets a user scan or filter locked rules and issues without reading a full text dump. Operations offers a native save-file picker for the backup destination, consistent with the workspace folder picker in Task 034. Source and edit views read like a competent editor. A diff viewer offers both an inline decorated view (added/removed/changed lines, for reviewing model-made changes in place) and a side-by-side comparison view (for evaluating working-tree/branch git state), reachable from the same documents already in the workbench. |
