...
| 015 | Partial | Register and trust a .NET workspace | Git inspection, durable registration, active selection, entry-point validation, and explicit trust exist behind services but are not exposed in the TUI or bound to the dashboard. | A user can add a Git repository, select a solution/project, explicitly trust it, reopen it, and see dirty/base state. |
...
- Task 015 has deterministic Git inspection, SQLite registry, single-active-workspace
  selection, entry-point validation, and explicit trust-transition coverage. Its
  end-user TUI workflow remains open.
