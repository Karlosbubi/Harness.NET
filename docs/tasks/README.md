...
| 040 | In progress | Collaborate through chat instead of a chain of pop-ups | 035 | Searchable Settings, persisted role defaults, read-only workflow cards, and deterministic composer-based private goal creation/inline continuation are delivered. Plan progression and authority actions still use `GoalDialog`; progressive goal overrides and dialog removal remain. | Conversation is the primary goal surface; typed inline cards expose plans, cost/capability decisions, progress, validation, evidence, Restore, commit, and handoff. Plan, spending, Restore, destructive, budget-extension, and exact-commit authority remain explicit typed actions. Ordinary role/model/output defaults move to one searchable Settings surface, goal overrides use progressive disclosure, and obsolete dialog paths are removed. |
