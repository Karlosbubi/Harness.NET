...
| 040 | In progress | Collaborate through chat instead of a chain of pop-ups | 035 | Searchable Settings, persisted role defaults, workflow cards, composer-based private goal entry/continuation, inline plan/run/Restore/exact-commit controls, and draft-only progressive goal limits/model routes are delivered. Semantic/detail surfaces still use `GoalDialog`, and obsolete orchestration paths remain to be removed. | Conversation is the primary goal surface; typed inline cards expose plans, cost/capability decisions, progress, validation, evidence, Restore, commit, and handoff. Plan, spending, Restore, destructive, budget-extension, and exact-commit authority remain explicit typed actions. Ordinary role/model/output defaults move to one searchable Settings surface, goal overrides use progressive disclosure, and obsolete dialog paths are removed. |
