...
| 040 | In progress | Collaborate through chat instead of a chain of pop-ups | 035 | Searchable Settings, persisted role defaults, workflow cards, deterministic composer-based private goal creation/inline continuation, and inline plan/run controls are delivered. Restore, commit, capability, and advanced override actions still use `GoalDialog`; progressive goal overrides and dialog removal remain. | Conversation is the primary goal surface; typed inline cards expose plans, cost/capability decisions, progress, validation, evidence, Restore, commit, and handoff. Plan, spending, Restore, destructive, budget-extension, and exact-commit authority remain explicit typed actions. Ordinary role/model/output defaults move to one searchable Settings surface, goal overrides use progressive disclosure, and obsolete dialog paths are removed. |
...
4. **Inline authority actions:** bind explicit typed approve/deny/cancel commands to
   ...
   Plan generation, plan approval/change requests, production continuation, and run
   cancellation now project through typed actions on their matching cards. Bounded
   role-call disclosures and the focused plan confirmation remain in force. Remote
   authorization, Restore, budget, destructive-operation, and exact-commit cards are
   still pending.
