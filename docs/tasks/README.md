...
- Treat configuration as product behavior: every configurable feature chunk ships its
  typed Settings owner, editable management surface, validation, persistence, status,
  and documentation in the same chunk. Hand-edited keys alone are not complete.
...
5. **Agent mutation preflight:** classify every candidate as compiler-validated or
   ...
   Complete: the agent edit tool marks model authority explicitly while manual editor
   saves remain permissive. Compiler-source replacements require an approved-worktree
   Roslyn session and exact baseline, compare stable retained/resolved/introduced
   diagnostics in an ephemeral solution, reject introduced compiler Errors before
   disk, retain warnings/analyzer findings in durable candidate evidence, and perform
   an applied-state pass after the atomic file replacement. Unsupported documentation
   is recorded as NotApplicable; known .NET project-system inputs that the current
   Roslyn document model cannot represent fail closed. Focused adapter and orchestration
   evidence is recorded in
   `docs/acceptance/roslyn-agent-edit-validation-2026-07-31.md`.
...
Complete: the Data Access mutation boundary now validates, stages, rechecks, and
commits up to 100 unique confined file replacements under one worktree gate, restoring
every original when a write or cancellation interrupts the commit. Roslyn resolves the
exact symbol and produces a local, 10 MiB-bounded preview with repository-relative
identity, original/new text, replacements, baselines, conflicts, diagnostic delta, and
stable fingerprint. Apply recomputes that preview, checks its fingerprint and normalized
human/Implementer grants, writes one atomic batch, verifies the persisted result through
Roslyn, and stores schema-20 rename evidence. F2 and the Implementer's closed preview
...
0. **Stateless MCP foundation (delivered):** pin the stable official C# MCP SDK 2.x,
   use Streamable HTTP and the `2026-07-28` stateless discovery lifecycle, normalize
   SDK types inside Data Access, and expose only explicitly read-only,
   non-destructive tools through namespaced agent functions. Interactive startup and
   explicit refresh discover without inference. The searchable Settings window adds,
   edits, enables/disables, removes, and reports named private-XDG connections from
   the same slice. Catalog and schema bounds prevent configured servers from growing
   ambient agent context without limit. ADR 015 records transport and authority
   boundaries.
...
recovery, explicit local-branch handoff, and verified in-app application-state
recovery. Task 038 remains explicitly deferred as an optional quality evaluation.
