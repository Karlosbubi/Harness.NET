...
grep -q 'Porta.Pty.*2.2.0' "$publish_root/THIRD-PARTY-NOTICES.md"
grep -q 'SvcSystems.UI.Terminal.*1.1.3' "$publish_root/THIRD-PARTY-NOTICES.md"
test -n "$(find "$publish_root" -maxdepth 1 -type f -name '*.so' -print -quit)"
test -f "$publish_root/libporta_pty.so"
