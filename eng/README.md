...
| `verify-local-model-regression.py` | Deterministic regression corpus; `--live` explicitly enables local Ollama comparisons, and repeatable `--reasoning-off-role` flags exercise persisted role opt-outs. | Python 3; Ollama only for explicit live mode. | Task 038/069 regression records. | <1 min fixtures; model-dependent live |
