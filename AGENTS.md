...
- Prefer `DataAccess -> BusinessLogic -> Presentation` layering where sensible.
  Only interfaces and records may cross those boundaries, and data/contracts flow
  upward except where dependency injection requires reverse-boundary composition.
...
- Keep provider SDK types inside Data Access and Microsoft Agent Framework types
  behind Business Logic interfaces and records.
