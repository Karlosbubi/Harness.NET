ALTER TABLE developer_dotnet_executions RENAME TO developer_dotnet_executions_v32;
...
CREATE TABLE developer_dotnet_executions (
    ...
    CHECK (
        (operation = 'Test' AND test_id IS NOT NULL AND test_name IS NOT NULL) OR
...
SELECT id, workspace_id, goal_id, source_description, project_path, target_framework,
       declaration_id, state, started_at, completed_at, exit_code, duration_milliseconds,
       error_code, error, operation, configuration, NULL, NULL
FROM developer_dotnet_executions_v32;

DROP TABLE developer_dotnet_executions_v32;
